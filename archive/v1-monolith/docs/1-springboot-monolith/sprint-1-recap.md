...
Credentials and local overrides are stored in `src/main/resources/application-local.properties` (git-ignored). Spring Boot activates this profile automatically because `application.properties` sets `spring.profiles.active=local`.

```properties
# application-local.properties  —  never committed
postgres_user_name=my_app_user
postgres_user_password=my_secure_password
postgres_db_name=cloud_native_db
```

The base `application.properties` references these variables and defines all other runtime settings:

```properties
spring.application.name=cloud-native-image-processing
...
> **Note:** `spring-dotenv` is no longer a dependency. Credential injection is handled natively by Spring Boot's profile mechanism — no extra library required.
...
Spring Boot loads `application.properties` first, then overlays `application-local.properties` (activated by `spring.profiles.active=local`). Hibernate creates or updates the schema on startup (`ddl-auto=create`). The API is available at `http://localhost:8080/api`.

To run the test suite against the local PostgreSQL instance:

```bash
# Run all tests including the full context load test
mvn clean test

# Run only unit and slice tests, skipping the context load test (no DB required)
mvn clean test -Dtest='!CloudNativeImageProcessingApplicationTests'
```
